...
**/
    ...
    if((node = malloc(sizeof *node)) == NULL) {
        fprintf(stderr, "%s: Unable to allocate memory for BST node\n", __func__);
        exit(1);
    }
...
**/
    ...
    while((n = STopPopP(&s)) != NULL) {
        Leftmost_Preorder(n->RPtr, &s);
    }
}
...
**/
    ...
    SPushP(&s, NULL);
    SPushP(&s, RootPtr);
    Leftmost_Inorder(RootPtr->LPtr, &s);

    while((n = STopPopP(&s)) != NULL) {
        BTWorkOut(n);
        Leftmost_Inorder(n->RPtr, &s);
    }
}
...
**/
    ...
    while((n = STopPopP(&sp)) != NULL) {
        ...
        if(!b) {
            ...
            SPushB(&sb, false);
            SPushP(&sp, n);
            Leftmost_Postorder(n->RPtr, &sp, &sb);
        }
    }
}
...
**/
    ...
    while((stackSize = SSizeP(sp1)) > 0) {
        height++;

        // Descent to next node layer
        while(stackSize > 0) {
            n = STopPopP(sp1);
