...
**/
    ...
    if(RootPtr->Key == K) {
        *Content = RootPtr->BSTNodeCont;
        ...
        rc = BSTSearch(RootPtr->LPtr, K, Content);
    } else {
        rc = BSTSearch(RootPtr->RPtr, K, Content);
    }
...
**/
    if(IsRoot == TRUE && NodePtr == NULL)
        return -1;
    else if(IsRoot == FALSE && NodePtr == NULL)
        return 0;

    int left = BSTHeight(NodePtr->LPtr, FALSE);
    int right = BSTHeight(NodePtr->RPtr, FALSE);
